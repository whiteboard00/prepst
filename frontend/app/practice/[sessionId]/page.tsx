...
function PracticeSessionContent() {
  ...
  const currentAnswer = currentQuestion ? answers[currentQuestion.question.id] : null;
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-background p-4">
        <Card className="w-full max-w-md">
          ...
          <CardContent>
            <Button onClick={() => router.push('/study-plan')} className="w-full">
              Back to Study Plan
            </Button>
          </CardContent>
        </Card>
          ...
          <div className="flex items-center justify-between mb-2">
            <div>
              ...
              <p className="text-sm text-muted-foreground">
                Question {currentIndex + 1} of {questions.length} • {currentQuestion.topic.name}
              </p>
            </div>
          ...
          <CardHeader>
            <div className="flex items-center justify-between mb-2">
              <span className={`px-2 py-1 rounded text-xs font-medium ${
                currentQuestion.question.difficulty === 'E' ? 'bg-green-100 text-green-800' :
                currentQuestion.question.difficulty === 'M' ? 'bg-yellow-100 text-yellow-800' :
                'bg-red-100 text-red-800'
              }`}>
                {currentQuestion.question.difficulty === 'E' ? 'Easy' :
                 currentQuestion.question.difficulty === 'M' ? 'Medium' : 'Hard'}
              </span>
              <span className="text-sm text-muted-foreground">
                {currentQuestion.question.question_type === 'mc' ? 'Multiple Choice' : 'Student Produced Response'}
              </span>
            </div>
          </CardHeader>
          <CardContent>
            ...
            <div
              ...
              dangerouslySetInnerHTML={{ __html: currentQuestion.question.stem }}
            />

            {/* Answer Options - Multiple Choice */}
            {currentQuestion.question.question_type === 'mc' && currentQuestion.question.answer_options && (
              <RadioGroup
              ...
              >
                {(() => {
                  ...
                  return options.map((option: any, index: number) => {
                    ...
                    const optionContent = option.content || option[1]?.content || option[1];

                    return (
                      <div key={optionId} className="flex items-center space-x-3 p-3 border rounded-lg hover:bg-accent">
                        <RadioGroupItem value={optionId} id={`option-${optionId}`} />
                        <Label
                          htmlFor={`option-${optionId}`}
                          className="flex-1 cursor-pointer"
                          dangerouslySetInnerHTML={{ __html: `<strong>${label}.</strong> ${optionContent}` }}
                        />
                      </div>
                    );
                  });
            ...
            {showFeedback && currentAnswer && (
              <div className={`mt-6 p-4 rounded-lg ${
                ...
                {!currentAnswer.isCorrect && (
                  <p className="text-sm text-muted-foreground">
                    Correct answer: {currentQuestion.question.correct_answer.join(', ')}
                  </p>
                )}
              </div>
            )}
